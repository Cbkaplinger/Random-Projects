...
Batter Graphs
```{r}
x = batter$`sprint_speed`
y = batter$`Contact_pct_(sc)`

line <-lm(formula = y ~ x, data=batter)   
intercept<-line$coefficients[1]
slope<- line$coefficients[2]

batter |> ggplot(aes(x = `sprint_speed`, y = `Contact_pct_(sc)`)) +
...
ggrepel:: geom_label_repel(aes(label = Name)) + theme_bw() + 
  ...
  labs(x = "Max Exit Velo", y = "BB%", title = "Max exit velo and BB%, 2023", caption = "Data: FanGraphs, Baseball Savant | @Cam_Kaplinger") + scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) + scale_y_continuous(breaks = scales::pretty_breaks(n = 8)) +
